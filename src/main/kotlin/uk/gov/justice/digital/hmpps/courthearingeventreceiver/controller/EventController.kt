...
class EventController(
  ...
  private fun trackAndSendEvent(hearingEventType: HearingEventType, hearingEvent: HearingEvent) {
    ...
    telemetryService.trackEvent(
      getTelemetryEventType(hearingEventType),
      mapOf(
  ...
  fun getTelemetryEventType(hearingEventType: HearingEventType): TelemetryEventType {
    return when (hearingEventType) {
      HearingEventType.CONFIRMED_OR_UPDATED -> TelemetryEventType.COURT_HEARING_UPDATE_EVENT_RECEIVED
      HearingEventType.RESULTED -> TelemetryEventType.COURT_HEARING_RESULT_EVENT_RECEIVED
      else -> {
        throw RuntimeException("Unknown hearing event type received: $hearingEventType")
      }
    }
  }
